...
from gymecs.raylib import *
import numpy
import time
import math

## Drawing components
class RayLibRectangle_EntityPosition(RayLibDrawComponent):
    """ Draw a cube based on the entity.position.position value.
    """    
    size:list=[1.0,1.0,1.0]
    color:list=[255,0,0,255]

    def _draw(self,entity):
        position = pr.Vector3(entity.position.x,entity.position.y,0.0)
        color = pr.Color(*self.color)
        rl.DrawCube(position, self.size[0],self.size[1],self.size[2], color)


class RayLibMazeWalls(RayLibDrawComponent):
    """ Draw a cube based on the entity.position.position value.
    """    
    size:list=[1.0,1.0,1.0]
    color:list=[64,64,64,255]
    ...
    def _draw(self,entity):
        ...
        for x in range(sx):
            for y in range(sy):
                if map[x,y]==1.0:
                    position = pr.Vector3(*[x,y,0.0])
                    color = pr.Color(*self.color)
                    rl.DrawCube(position, self.size[0],self.size[1],self.size[2], color)
                    
class RayLibMazeGround(RayLibDrawComponent):
    """ Draw a cube based on the entity.position.position value.
    """    
    size:list=[1.0,1.0,1.0]
    color:list=[64,255,255,255]

    def _draw(self,entity):
        map=entity.map.map
        sx,sy=map.shape
        position = pr.Vector3(*[sx/2.0,sy/2.0,-0.5])
        color = pr.Color(*self.color)
        rl.DrawCube(position, sx,sy,0.01, color)
...
class E_Agent(Entity):
    ...
    cube: RayLibRectangle_EntityPosition = RayLibRectangle_EntityPosition()

class E_Maze(Entity):
    map:MazeMap=MazeMap()
    size:Size=Size()
    draw_walls=RayLibMazeWalls()
    draw_ground=RayLibMazeGround()

class E_Goal(Entity):
    position:Position=Position()
    cube: RayLibRectangle_EntityPosition = RayLibRectangle_EntityPosition(color=[0,255,0,255])
...
class ControlCamera(System):
    def __call__(self,world,**arguments):
        timestep=world.get_entity('game').infos.timestep

        c,s=math.cos(timestep/180*math.pi),math.sin(timestep/180*math.pi)
        c=c*30
        s=s*30
        camera=world.get_entity('game').camera._structureclone(position=[c,s,18.0])
        world.set_component('game','camera',camera)
...
class MultiAgentMazeGame(Game):
    def __init__(self,size=(21,21),wall_density=0.1,n_agents=4):        
        ...
        self._control_camera=ControlCamera()
        self._players=[]
        self._render_system = RayLibSystem(
            screen_size=(640, 480), screen_title="RayLib Maze", debug=False
        )
    ...
    def reset(self,seed):
        ...
        game=E_GameState()
        game=game._structureclone(camera=RayLibCamera(target=[self._size[0]/2.0,self._size[1]/2.0,0.0]))
        ...
        self._control_camera(self._world)
        self._render_system(self._world,_game_dt=0.1)
        return self._world
    ...
    def step(self,**arguments):
        ...
        for p in self._players:
            p(self._world)
        self._control_camera(self._world)
        self._render_system(self._world)
